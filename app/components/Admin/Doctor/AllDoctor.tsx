...
} from "@/redux/features/doctors/doctorApi";
...
const AllDoctors = (props: Props) => {
  const { theme, setTheme } = useTheme();
  const [open, setOpen] = useState(false);
  const [doctorId, setDoctorId] = useState("");
  const { isLoading, data, refetch } = useGetAllDoctorsQuery(
    {},
    { refetchOnMountOrArgChange: true }
  );
  const [deleteDoctor, { isSuccess, error }] = useDeleteDoctorMutation({});
  const columns = [
    ...
    { field: "created_at", headerName: "Created At", flex: 0.5 },
    {
      ...
      renderCell: (params: any) => {
        return (
          <>
            <Link href={`/admin/edit-doctor/${params.row.id}`}>
