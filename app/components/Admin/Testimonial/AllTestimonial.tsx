...
const AllTestimonials = (props: Props) => {
  const { theme, setTheme } = useTheme();
  const [open, setOpen] = useState(false);
  const [testimonialId, setTestimonialId] = useState("");
  const { isLoading, data, refetch } = useGetAllTestimonialsQuery(
    {},
    { refetchOnMountOrArgChange: true }
  );
  const [deleteTestimonial, { isSuccess, error }] = useDeleteTestimonialMutation({});
  const columns = [
    ...
    { field: "created_at", headerName: "Created At", flex: 0.5 },
    {
      ...
      renderCell: (params: any) => {
        return (
          <>
            <Link href={`/admin/edit-testimonial/${params.row.id}`}>
        ...
        return (
          <>
            <Button
              onClick={() => {
                setOpen(!open);
                setTestimonialId(params.row.id);
              }}
            >
              <AiOutlineDelete
